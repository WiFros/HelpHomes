...
function insert(board, success, fail) {
  ...
  axios.post('/qna', board).then(success).catch(fail);
}
...
function updateAnswer(board, success, fail) {
  const num = board.num;
  axios
    .put('/qna/answer/' + num, board)
    .then(success)
    .catch(fail);
}
...
export { selectAll, selectOne, insert, update, remove, search, updateAnswer };
